...
\begin{alectryon}
  ...
  \begin{sentence}
    \begin{input}
      \PY{k+kn}{Goal}~\PY{k+kr}{exists}~\PY{n+nv}{x}\PY{o}{,}~\PY{n}{x}~\PY{o}{*}~\PY{n}{x}~\PY{o}{=}~\PY{l+m+mi}{49}~\PY{o}{/\PYZbs{}}~\PY{n}{x}~\PY{o}{\PYZlt{}}~\PY{l+m+mi}{10}\PY{o}{.}
    \end{input}
    \sep
    \begin{output}
      \begin{goals}
        \begin{goal}
          ...
          \sep
          \begin{conclusion}
            \PY{k+kr}{exists}~\PY{n+nv}{x}~\PY{o}{:}~\PY{n}{nat}\PY{o}{,}~\PY{n}{x}~\PY{o}{*}~\PY{n}{x}~\PY{o}{=}~\PY{l+m+mi}{49}~\PY{o}{/\PYZbs{}}~\PY{n}{x}~\PY{o}{\PYZlt{}}~\PY{l+m+mi}{10}
          \end{conclusion}
        \end{goal}
      \end{goals}
    \end{output}
  ...
  \begin{sentence}
    \begin{input}
      ~~\PY{k+kr}{let~rec}~\PY{n+nv}{t}~\PY{n+nv}{n}~\PY{o}{:=}\nl
      ~~~~\PY{o}{(}\PY{k+kr}{exists}~\PY{n+nv}{n}\PY{o}{;}~\PY{n+nb}{split}\PY{o}{;}~\PY{o}{[}\PY{n+nb+bp}{reflexivity}~\PY{o}{|])}~\PY{o}{||}~\PY{n}{t}~\PY{o}{(}\PY{n}{S}~\PY{n}{n}\PY{o}{)}~\PY{k+kr}{in}\nl
...
\begin{alectryon}
  ...
  \begin{sentence}
    \begin{input}
      \PY{k+kn}{Require~Import}~\PY{n}{ZArith}\PY{o}{.}
    \end{input}
  \end{sentence}
\end{alectryon}
...
\begin{alectryon}
  ...
  \begin{sentence}
    \begin{input}
      \PY{k+kn}{Fail}~\PY{k+kn}{Check}~\PY{n}{z}~\PY{o}{+}~\PY{l+m+mi}{0}\PY{o}{\PYZpc{}}\PY{n}{nat}\PY{o}{.}
    \end{input}
    \sep
    \begin{output}
      \begin{messages}
        \begin{message}
          \PY{n}{The}~\PY{n}{command}~\PY{n}{has}~\PY{n}{indeed}~\PY{n}{failed}~\PY{k+kr}{with}~\PY{n}{message}\PY{o}{:}\nl
          \PY{n}{The}~\PY{n}{reference}~\PY{n}{z}~\PY{n}{was}~\PY{n}{not}~\PY{n}{found}~\PY{k+kr}{in}~\PY{n}{the}~\PY{n}{current}\nl
